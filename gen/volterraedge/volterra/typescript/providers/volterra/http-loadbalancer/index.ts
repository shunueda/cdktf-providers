// generated from terraform resource schema

import { HttpLoadbalancerActiveServicePolicies, 
httpLoadbalancerActiveServicePoliciesToTerraform, 
httpLoadbalancerActiveServicePoliciesToHclTerraform, 
HttpLoadbalancerActiveServicePoliciesOutputReference, 
HttpLoadbalancerAdvertiseCustom, 
httpLoadbalancerAdvertiseCustomToTerraform, 
httpLoadbalancerAdvertiseCustomToHclTerraform, 
HttpLoadbalancerAdvertiseCustomOutputReference, 
HttpLoadbalancerAdvertiseOnPublic, 
httpLoadbalancerAdvertiseOnPublicToTerraform, 
httpLoadbalancerAdvertiseOnPublicToHclTerraform, 
HttpLoadbalancerAdvertiseOnPublicOutputReference, 
HttpLoadbalancerApiProtectionRules, 
httpLoadbalancerApiProtectionRulesToTerraform, 
httpLoadbalancerApiProtectionRulesToHclTerraform, 
HttpLoadbalancerApiProtectionRulesOutputReference, 
HttpLoadbalancerApiRateLimit, 
httpLoadbalancerApiRateLimitToTerraform, 
httpLoadbalancerApiRateLimitToHclTerraform, 
HttpLoadbalancerApiRateLimitOutputReference, 
HttpLoadbalancerApiSpecification, 
httpLoadbalancerApiSpecificationToTerraform, 
httpLoadbalancerApiSpecificationToHclTerraform, 
HttpLoadbalancerApiSpecificationOutputReference, 
HttpLoadbalancerApiTesting, 
httpLoadbalancerApiTestingToTerraform, 
httpLoadbalancerApiTestingToHclTerraform, 
HttpLoadbalancerApiTestingOutputReference, 
HttpLoadbalancerAppFirewall, 
httpLoadbalancerAppFirewallToTerraform, 
httpLoadbalancerAppFirewallToHclTerraform, 
HttpLoadbalancerAppFirewallOutputReference, 
HttpLoadbalancerBlockedClients, 
httpLoadbalancerBlockedClientsToTerraform, 
httpLoadbalancerBlockedClientsToHclTerraform, 
HttpLoadbalancerBlockedClientsList, 
HttpLoadbalancerBotDefense, 
httpLoadbalancerBotDefenseToTerraform, 
httpLoadbalancerBotDefenseToHclTerraform, 
HttpLoadbalancerBotDefenseOutputReference, 
HttpLoadbalancerCaptchaChallenge, 
httpLoadbalancerCaptchaChallengeToTerraform, 
httpLoadbalancerCaptchaChallengeToHclTerraform, 
HttpLoadbalancerCaptchaChallengeOutputReference, 
HttpLoadbalancerClientSideDefense, 
httpLoadbalancerClientSideDefenseToTerraform, 
httpLoadbalancerClientSideDefenseToHclTerraform, 
HttpLoadbalancerClientSideDefenseOutputReference, 
HttpLoadbalancerCookieStickiness, 
httpLoadbalancerCookieStickinessToTerraform, 
httpLoadbalancerCookieStickinessToHclTerraform, 
HttpLoadbalancerCookieStickinessOutputReference, 
HttpLoadbalancerCorsPolicy, 
httpLoadbalancerCorsPolicyToTerraform, 
httpLoadbalancerCorsPolicyToHclTerraform, 
HttpLoadbalancerCorsPolicyOutputReference, 
HttpLoadbalancerCsrfPolicy, 
httpLoadbalancerCsrfPolicyToTerraform, 
httpLoadbalancerCsrfPolicyToHclTerraform, 
HttpLoadbalancerCsrfPolicyOutputReference, 
HttpLoadbalancerDataGuardRules, 
httpLoadbalancerDataGuardRulesToTerraform, 
httpLoadbalancerDataGuardRulesToHclTerraform, 
HttpLoadbalancerDataGuardRulesList, 
HttpLoadbalancerDdosMitigationRules, 
httpLoadbalancerDdosMitigationRulesToTerraform, 
httpLoadbalancerDdosMitigationRulesToHclTerraform, 
HttpLoadbalancerDdosMitigationRulesList, 
HttpLoadbalancerDefaultRoutePools, 
httpLoadbalancerDefaultRoutePoolsToTerraform, 
httpLoadbalancerDefaultRoutePoolsToHclTerraform, 
HttpLoadbalancerDefaultRoutePoolsList, 
HttpLoadbalancerEnableApiDiscovery, 
httpLoadbalancerEnableApiDiscoveryToTerraform, 
httpLoadbalancerEnableApiDiscoveryToHclTerraform, 
HttpLoadbalancerEnableApiDiscoveryOutputReference, 
HttpLoadbalancerEnableChallenge, 
httpLoadbalancerEnableChallengeToTerraform, 
httpLoadbalancerEnableChallengeToHclTerraform, 
HttpLoadbalancerEnableChallengeOutputReference, 
HttpLoadbalancerEnableIpReputation, 
httpLoadbalancerEnableIpReputationToTerraform, 
httpLoadbalancerEnableIpReputationToHclTerraform, 
HttpLoadbalancerEnableIpReputationOutputReference, 
HttpLoadbalancerEnableTrustClientIpHeaders, 
httpLoadbalancerEnableTrustClientIpHeadersToTerraform, 
httpLoadbalancerEnableTrustClientIpHeadersToHclTerraform, 
HttpLoadbalancerEnableTrustClientIpHeadersOutputReference, 
HttpLoadbalancerGraphqlRules, 
httpLoadbalancerGraphqlRulesToTerraform, 
httpLoadbalancerGraphqlRulesToHclTerraform, 
HttpLoadbalancerGraphqlRulesList, 
HttpLoadbalancerHttp, 
httpLoadbalancerHttpToTerraform, 
httpLoadbalancerHttpToHclTerraform, 
HttpLoadbalancerHttpOutputReference, 
HttpLoadbalancerHttps, 
httpLoadbalancerHttpsToTerraform, 
httpLoadbalancerHttpsToHclTerraform, 
HttpLoadbalancerHttpsOutputReference, 
HttpLoadbalancerHttpsAutoCert, 
httpLoadbalancerHttpsAutoCertToTerraform, 
httpLoadbalancerHttpsAutoCertToHclTerraform, 
HttpLoadbalancerHttpsAutoCertOutputReference, 
HttpLoadbalancerJsChallenge, 
httpLoadbalancerJsChallengeToTerraform, 
httpLoadbalancerJsChallengeToHclTerraform, 
HttpLoadbalancerJsChallengeOutputReference, 
HttpLoadbalancerJwtValidation, 
httpLoadbalancerJwtValidationToTerraform, 
httpLoadbalancerJwtValidationToHclTerraform, 
HttpLoadbalancerJwtValidationOutputReference, 
HttpLoadbalancerL7DdosProtection, 
httpLoadbalancerL7DdosProtectionToTerraform, 
httpLoadbalancerL7DdosProtectionToHclTerraform, 
HttpLoadbalancerL7DdosProtectionOutputReference, 
HttpLoadbalancerMalwareProtectionSettings, 
httpLoadbalancerMalwareProtectionSettingsToTerraform, 
httpLoadbalancerMalwareProtectionSettingsToHclTerraform, 
HttpLoadbalancerMalwareProtectionSettingsOutputReference, 
HttpLoadbalancerMoreOption, 
httpLoadbalancerMoreOptionToTerraform, 
httpLoadbalancerMoreOptionToHclTerraform, 
HttpLoadbalancerMoreOptionOutputReference, 
HttpLoadbalancerOriginServerSubsetRuleListStruct, 
httpLoadbalancerOriginServerSubsetRuleListStructToTerraform, 
httpLoadbalancerOriginServerSubsetRuleListStructToHclTerraform, 
HttpLoadbalancerOriginServerSubsetRuleListStructOutputReference, 
HttpLoadbalancerPolicyBasedChallenge, 
httpLoadbalancerPolicyBasedChallengeToTerraform, 
httpLoadbalancerPolicyBasedChallengeToHclTerraform, 
HttpLoadbalancerPolicyBasedChallengeOutputReference, 
HttpLoadbalancerProtectedCookies, 
httpLoadbalancerProtectedCookiesToTerraform, 
httpLoadbalancerProtectedCookiesToHclTerraform, 
HttpLoadbalancerProtectedCookiesList, 
HttpLoadbalancerRateLimit, 
httpLoadbalancerRateLimitToTerraform, 
httpLoadbalancerRateLimitToHclTerraform, 
HttpLoadbalancerRateLimitOutputReference, 
HttpLoadbalancerRingHash, 
httpLoadbalancerRingHashToTerraform, 
httpLoadbalancerRingHashToHclTerraform, 
HttpLoadbalancerRingHashOutputReference, 
HttpLoadbalancerRoutes, 
httpLoadbalancerRoutesToTerraform, 
httpLoadbalancerRoutesToHclTerraform, 
HttpLoadbalancerRoutesList, 
HttpLoadbalancerSensitiveDataDisclosureRules, 
httpLoadbalancerSensitiveDataDisclosureRulesToTerraform, 
httpLoadbalancerSensitiveDataDisclosureRulesToHclTerraform, 
HttpLoadbalancerSensitiveDataDisclosureRulesOutputReference, 
HttpLoadbalancerSensitiveDataPolicy, 
httpLoadbalancerSensitiveDataPolicyToTerraform, 
httpLoadbalancerSensitiveDataPolicyToHclTerraform, 
HttpLoadbalancerSensitiveDataPolicyOutputReference, 
HttpLoadbalancerSlowDdosMitigation, 
httpLoadbalancerSlowDdosMitigationToTerraform, 
httpLoadbalancerSlowDdosMitigationToHclTerraform, 
HttpLoadbalancerSlowDdosMitigationOutputReference, 
HttpLoadbalancerTrustedClients, 
httpLoadbalancerTrustedClientsToTerraform, 
httpLoadbalancerTrustedClientsToHclTerraform, 
HttpLoadbalancerTrustedClientsList, 
HttpLoadbalancerUserIdentification, 
httpLoadbalancerUserIdentificationToTerraform, 
httpLoadbalancerUserIdentificationToHclTerraform, 
HttpLoadbalancerUserIdentificationOutputReference, 
HttpLoadbalancerWafExclusionRules, 
httpLoadbalancerWafExclusionRulesToTerraform, 
httpLoadbalancerWafExclusionRulesToHclTerraform, 
HttpLoadbalancerWafExclusionRulesList} from './index-structs'
export * from './index-structs'
import { Construct } from 'constructs';
import * as cdktf from 'cdktf';
export interface HttpLoadbalancerConfig extends cdktf.TerraformMetaArguments {
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#add_location HttpLoadbalancer#add_location}
  */
  readonly addLocation?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#advertise_on_public_default_vip HttpLoadbalancer#advertise_on_public_default_vip}
  */
  readonly advertiseOnPublicDefaultVip?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#annotations HttpLoadbalancer#annotations}
  */
  readonly annotations?: { [key: string]: string };
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#default_sensitive_data_policy HttpLoadbalancer#default_sensitive_data_policy}
  */
  readonly defaultSensitiveDataPolicy?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#description HttpLoadbalancer#description}
  */
  readonly description?: string;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#disable HttpLoadbalancer#disable}
  */
  readonly disable?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#disable_api_definition HttpLoadbalancer#disable_api_definition}
  */
  readonly disableApiDefinition?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#disable_api_discovery HttpLoadbalancer#disable_api_discovery}
  */
  readonly disableApiDiscovery?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#disable_api_testing HttpLoadbalancer#disable_api_testing}
  */
  readonly disableApiTesting?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#disable_bot_defense HttpLoadbalancer#disable_bot_defense}
  */
  readonly disableBotDefense?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#disable_client_side_defense HttpLoadbalancer#disable_client_side_defense}
  */
  readonly disableClientSideDefense?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#disable_ip_reputation HttpLoadbalancer#disable_ip_reputation}
  */
  readonly disableIpReputation?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#disable_malicious_user_detection HttpLoadbalancer#disable_malicious_user_detection}
  */
  readonly disableMaliciousUserDetection?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#disable_malware_protection HttpLoadbalancer#disable_malware_protection}
  */
  readonly disableMalwareProtection?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#disable_rate_limit HttpLoadbalancer#disable_rate_limit}
  */
  readonly disableRateLimit?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#disable_threat_mesh HttpLoadbalancer#disable_threat_mesh}
  */
  readonly disableThreatMesh?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#disable_trust_client_ip_headers HttpLoadbalancer#disable_trust_client_ip_headers}
  */
  readonly disableTrustClientIpHeaders?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#disable_waf HttpLoadbalancer#disable_waf}
  */
  readonly disableWaf?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#do_not_advertise HttpLoadbalancer#do_not_advertise}
  */
  readonly doNotAdvertise?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#domains HttpLoadbalancer#domains}
  */
  readonly domains: string[];
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#enable_malicious_user_detection HttpLoadbalancer#enable_malicious_user_detection}
  */
  readonly enableMaliciousUserDetection?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#enable_threat_mesh HttpLoadbalancer#enable_threat_mesh}
  */
  readonly enableThreatMesh?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#id HttpLoadbalancer#id}
  *
  * Please be aware that the id field is automatically added to all resources in Terraform providers using a Terraform provider SDK version below 2.
  * If you experience problems setting this value it might not be settable. Please take a look at the provider documentation to ensure it should be settable.
  */
  readonly id?: string;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#labels HttpLoadbalancer#labels}
  */
  readonly labels?: { [key: string]: string };
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#least_active HttpLoadbalancer#least_active}
  */
  readonly leastActive?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#name HttpLoadbalancer#name}
  */
  readonly name: string;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#namespace HttpLoadbalancer#namespace}
  */
  readonly namespace: string;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#no_challenge HttpLoadbalancer#no_challenge}
  */
  readonly noChallenge?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#no_service_policies HttpLoadbalancer#no_service_policies}
  */
  readonly noServicePolicies?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#random HttpLoadbalancer#random}
  */
  readonly random?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#round_robin HttpLoadbalancer#round_robin}
  */
  readonly roundRobin?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#service_policies_from_namespace HttpLoadbalancer#service_policies_from_namespace}
  */
  readonly servicePoliciesFromNamespace?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#source_ip_stickiness HttpLoadbalancer#source_ip_stickiness}
  */
  readonly sourceIpStickiness?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#system_default_timeouts HttpLoadbalancer#system_default_timeouts}
  */
  readonly systemDefaultTimeouts?: boolean | cdktf.IResolvable;
  /**
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#user_id_client_ip HttpLoadbalancer#user_id_client_ip}
  */
  readonly userIdClientIp?: boolean | cdktf.IResolvable;
  /**
  * active_service_policies block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#active_service_policies HttpLoadbalancer#active_service_policies}
  */
  readonly activeServicePolicies?: HttpLoadbalancerActiveServicePolicies;
  /**
  * advertise_custom block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#advertise_custom HttpLoadbalancer#advertise_custom}
  */
  readonly advertiseCustom?: HttpLoadbalancerAdvertiseCustom;
  /**
  * advertise_on_public block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#advertise_on_public HttpLoadbalancer#advertise_on_public}
  */
  readonly advertiseOnPublic?: HttpLoadbalancerAdvertiseOnPublic;
  /**
  * api_protection_rules block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#api_protection_rules HttpLoadbalancer#api_protection_rules}
  */
  readonly apiProtectionRules?: HttpLoadbalancerApiProtectionRules;
  /**
  * api_rate_limit block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#api_rate_limit HttpLoadbalancer#api_rate_limit}
  */
  readonly apiRateLimit?: HttpLoadbalancerApiRateLimit;
  /**
  * api_specification block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#api_specification HttpLoadbalancer#api_specification}
  */
  readonly apiSpecification?: HttpLoadbalancerApiSpecification;
  /**
  * api_testing block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#api_testing HttpLoadbalancer#api_testing}
  */
  readonly apiTesting?: HttpLoadbalancerApiTesting;
  /**
  * app_firewall block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#app_firewall HttpLoadbalancer#app_firewall}
  */
  readonly appFirewall?: HttpLoadbalancerAppFirewall;
  /**
  * blocked_clients block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#blocked_clients HttpLoadbalancer#blocked_clients}
  */
  readonly blockedClients?: HttpLoadbalancerBlockedClients[] | cdktf.IResolvable;
  /**
  * bot_defense block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#bot_defense HttpLoadbalancer#bot_defense}
  */
  readonly botDefense?: HttpLoadbalancerBotDefense;
  /**
  * captcha_challenge block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#captcha_challenge HttpLoadbalancer#captcha_challenge}
  */
  readonly captchaChallenge?: HttpLoadbalancerCaptchaChallenge;
  /**
  * client_side_defense block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#client_side_defense HttpLoadbalancer#client_side_defense}
  */
  readonly clientSideDefense?: HttpLoadbalancerClientSideDefense;
  /**
  * cookie_stickiness block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#cookie_stickiness HttpLoadbalancer#cookie_stickiness}
  */
  readonly cookieStickiness?: HttpLoadbalancerCookieStickiness;
  /**
  * cors_policy block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#cors_policy HttpLoadbalancer#cors_policy}
  */
  readonly corsPolicy?: HttpLoadbalancerCorsPolicy;
  /**
  * csrf_policy block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#csrf_policy HttpLoadbalancer#csrf_policy}
  */
  readonly csrfPolicy?: HttpLoadbalancerCsrfPolicy;
  /**
  * data_guard_rules block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#data_guard_rules HttpLoadbalancer#data_guard_rules}
  */
  readonly dataGuardRules?: HttpLoadbalancerDataGuardRules[] | cdktf.IResolvable;
  /**
  * ddos_mitigation_rules block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#ddos_mitigation_rules HttpLoadbalancer#ddos_mitigation_rules}
  */
  readonly ddosMitigationRules?: HttpLoadbalancerDdosMitigationRules[] | cdktf.IResolvable;
  /**
  * default_route_pools block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#default_route_pools HttpLoadbalancer#default_route_pools}
  */
  readonly defaultRoutePools?: HttpLoadbalancerDefaultRoutePools[] | cdktf.IResolvable;
  /**
  * enable_api_discovery block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#enable_api_discovery HttpLoadbalancer#enable_api_discovery}
  */
  readonly enableApiDiscovery?: HttpLoadbalancerEnableApiDiscovery;
  /**
  * enable_challenge block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#enable_challenge HttpLoadbalancer#enable_challenge}
  */
  readonly enableChallenge?: HttpLoadbalancerEnableChallenge;
  /**
  * enable_ip_reputation block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#enable_ip_reputation HttpLoadbalancer#enable_ip_reputation}
  */
  readonly enableIpReputation?: HttpLoadbalancerEnableIpReputation;
  /**
  * enable_trust_client_ip_headers block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#enable_trust_client_ip_headers HttpLoadbalancer#enable_trust_client_ip_headers}
  */
  readonly enableTrustClientIpHeaders?: HttpLoadbalancerEnableTrustClientIpHeaders;
  /**
  * graphql_rules block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#graphql_rules HttpLoadbalancer#graphql_rules}
  */
  readonly graphqlRules?: HttpLoadbalancerGraphqlRules[] | cdktf.IResolvable;
  /**
  * http block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#http HttpLoadbalancer#http}
  */
  readonly http?: HttpLoadbalancerHttp;
  /**
  * https block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#https HttpLoadbalancer#https}
  */
  readonly https?: HttpLoadbalancerHttps;
  /**
  * https_auto_cert block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#https_auto_cert HttpLoadbalancer#https_auto_cert}
  */
  readonly httpsAutoCert?: HttpLoadbalancerHttpsAutoCert;
  /**
  * js_challenge block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#js_challenge HttpLoadbalancer#js_challenge}
  */
  readonly jsChallenge?: HttpLoadbalancerJsChallenge;
  /**
  * jwt_validation block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#jwt_validation HttpLoadbalancer#jwt_validation}
  */
  readonly jwtValidation?: HttpLoadbalancerJwtValidation;
  /**
  * l7_ddos_protection block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#l7_ddos_protection HttpLoadbalancer#l7_ddos_protection}
  */
  readonly l7DdosProtection?: HttpLoadbalancerL7DdosProtection;
  /**
  * malware_protection_settings block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#malware_protection_settings HttpLoadbalancer#malware_protection_settings}
  */
  readonly malwareProtectionSettings?: HttpLoadbalancerMalwareProtectionSettings;
  /**
  * more_option block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#more_option HttpLoadbalancer#more_option}
  */
  readonly moreOption?: HttpLoadbalancerMoreOption;
  /**
  * origin_server_subset_rule_list block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#origin_server_subset_rule_list HttpLoadbalancer#origin_server_subset_rule_list}
  */
  readonly originServerSubsetRuleList?: HttpLoadbalancerOriginServerSubsetRuleListStruct;
  /**
  * policy_based_challenge block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#policy_based_challenge HttpLoadbalancer#policy_based_challenge}
  */
  readonly policyBasedChallenge?: HttpLoadbalancerPolicyBasedChallenge;
  /**
  * protected_cookies block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#protected_cookies HttpLoadbalancer#protected_cookies}
  */
  readonly protectedCookies?: HttpLoadbalancerProtectedCookies[] | cdktf.IResolvable;
  /**
  * rate_limit block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#rate_limit HttpLoadbalancer#rate_limit}
  */
  readonly rateLimit?: HttpLoadbalancerRateLimit;
  /**
  * ring_hash block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#ring_hash HttpLoadbalancer#ring_hash}
  */
  readonly ringHash?: HttpLoadbalancerRingHash;
  /**
  * routes block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#routes HttpLoadbalancer#routes}
  */
  readonly routes?: HttpLoadbalancerRoutes[] | cdktf.IResolvable;
  /**
  * sensitive_data_disclosure_rules block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#sensitive_data_disclosure_rules HttpLoadbalancer#sensitive_data_disclosure_rules}
  */
  readonly sensitiveDataDisclosureRules?: HttpLoadbalancerSensitiveDataDisclosureRules;
  /**
  * sensitive_data_policy block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#sensitive_data_policy HttpLoadbalancer#sensitive_data_policy}
  */
  readonly sensitiveDataPolicy?: HttpLoadbalancerSensitiveDataPolicy;
  /**
  * slow_ddos_mitigation block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#slow_ddos_mitigation HttpLoadbalancer#slow_ddos_mitigation}
  */
  readonly slowDdosMitigation?: HttpLoadbalancerSlowDdosMitigation;
  /**
  * trusted_clients block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#trusted_clients HttpLoadbalancer#trusted_clients}
  */
  readonly trustedClients?: HttpLoadbalancerTrustedClients[] | cdktf.IResolvable;
  /**
  * user_identification block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#user_identification HttpLoadbalancer#user_identification}
  */
  readonly userIdentification?: HttpLoadbalancerUserIdentification;
  /**
  * waf_exclusion_rules block
  *
  * Docs at Terraform Registry: {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#waf_exclusion_rules HttpLoadbalancer#waf_exclusion_rules}
  */
  readonly wafExclusionRules?: HttpLoadbalancerWafExclusionRules[] | cdktf.IResolvable;
}

/**
* Represents a {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer volterra_http_loadbalancer}
*/
export class HttpLoadbalancer extends cdktf.TerraformResource {

  // =================
  // STATIC PROPERTIES
  // =================
  public static readonly tfResourceType = "volterra_http_loadbalancer";

  // ==============
  // STATIC Methods
  // ==============
  /**
  * Generates CDKTF code for importing a HttpLoadbalancer resource upon running "cdktf plan <stack-name>"
  * @param scope The scope in which to define this construct
  * @param importToId The construct id used in the generated config for the HttpLoadbalancer to import
  * @param importFromId The id of the existing HttpLoadbalancer that should be imported. Refer to the {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer#import import section} in the documentation of this resource for the id to use
  * @param provider? Optional instance of the provider where the HttpLoadbalancer to import is found
  */
  public static generateConfigForImport(scope: Construct, importToId: string, importFromId: string, provider?: cdktf.TerraformProvider) {
        return new cdktf.ImportableResource(scope, importToId, { terraformResourceType: "volterra_http_loadbalancer", importId: importFromId, provider });
      }

  // ===========
  // INITIALIZER
  // ===========

  /**
  * Create a new {@link https://registry.terraform.io/providers/volterraedge/volterra/0.11.44/docs/resources/http_loadbalancer volterra_http_loadbalancer} Resource
  *
  * @param scope The scope in which to define this construct
  * @param id The scoped construct ID. Must be unique amongst siblings in the same scope
  * @param options HttpLoadbalancerConfig
  */
  public constructor(scope: Construct, id: string, config: HttpLoadbalancerConfig) {
    super(scope, id, {
      terraformResourceType: 'volterra_http_loadbalancer',
      terraformGeneratorMetadata: {
        providerName: 'volterra',
        providerVersion: '0.11.44',
        providerVersionConstraint: '0.11.44'
      },
      provider: config.provider,
      dependsOn: config.dependsOn,
      count: config.count,
      lifecycle: config.lifecycle,
      provisioners: config.provisioners,
      connection: config.connection,
      forEach: config.forEach
    });
    this._addLocation = config.addLocation;
    this._advertiseOnPublicDefaultVip = config.advertiseOnPublicDefaultVip;
    this._annotations = config.annotations;
    this._defaultSensitiveDataPolicy = config.defaultSensitiveDataPolicy;
    this._description = config.description;
    this._disable = config.disable;
    this._disableApiDefinition = config.disableApiDefinition;
    this._disableApiDiscovery = config.disableApiDiscovery;
    this._disableApiTesting = config.disableApiTesting;
    this._disableBotDefense = config.disableBotDefense;
    this._disableClientSideDefense = config.disableClientSideDefense;
    this._disableIpReputation = config.disableIpReputation;
    this._disableMaliciousUserDetection = config.disableMaliciousUserDetection;
    this._disableMalwareProtection = config.disableMalwareProtection;
    this._disableRateLimit = config.disableRateLimit;
    this._disableThreatMesh = config.disableThreatMesh;
    this._disableTrustClientIpHeaders = config.disableTrustClientIpHeaders;
    this._disableWaf = config.disableWaf;
    this._doNotAdvertise = config.doNotAdvertise;
    this._domains = config.domains;
    this._enableMaliciousUserDetection = config.enableMaliciousUserDetection;
    this._enableThreatMesh = config.enableThreatMesh;
    this._id = config.id;
    this._labels = config.labels;
    this._leastActive = config.leastActive;
    this._name = config.name;
    this._namespace = config.namespace;
    this._noChallenge = config.noChallenge;
    this._noServicePolicies = config.noServicePolicies;
    this._random = config.random;
    this._roundRobin = config.roundRobin;
    this._servicePoliciesFromNamespace = config.servicePoliciesFromNamespace;
    this._sourceIpStickiness = config.sourceIpStickiness;
    this._systemDefaultTimeouts = config.systemDefaultTimeouts;
    this._userIdClientIp = config.userIdClientIp;
    this._activeServicePolicies.internalValue = config.activeServicePolicies;
    this._advertiseCustom.internalValue = config.advertiseCustom;
    this._advertiseOnPublic.internalValue = config.advertiseOnPublic;
    this._apiProtectionRules.internalValue = config.apiProtectionRules;
    this._apiRateLimit.internalValue = config.apiRateLimit;
    this._apiSpecification.internalValue = config.apiSpecification;
    this._apiTesting.internalValue = config.apiTesting;
    this._appFirewall.internalValue = config.appFirewall;
    this._blockedClients.internalValue = config.blockedClients;
    this._botDefense.internalValue = config.botDefense;
    this._captchaChallenge.internalValue = config.captchaChallenge;
    this._clientSideDefense.internalValue = config.clientSideDefense;
    this._cookieStickiness.internalValue = config.cookieStickiness;
    this._corsPolicy.internalValue = config.corsPolicy;
    this._csrfPolicy.internalValue = config.csrfPolicy;
    this._dataGuardRules.internalValue = config.dataGuardRules;
    this._ddosMitigationRules.internalValue = config.ddosMitigationRules;
    this._defaultRoutePools.internalValue = config.defaultRoutePools;
    this._enableApiDiscovery.internalValue = config.enableApiDiscovery;
    this._enableChallenge.internalValue = config.enableChallenge;
    this._enableIpReputation.internalValue = config.enableIpReputation;
    this._enableTrustClientIpHeaders.internalValue = config.enableTrustClientIpHeaders;
    this._graphqlRules.internalValue = config.graphqlRules;
    this._http.internalValue = config.http;
    this._https.internalValue = config.https;
    this._httpsAutoCert.internalValue = config.httpsAutoCert;
    this._jsChallenge.internalValue = config.jsChallenge;
    this._jwtValidation.internalValue = config.jwtValidation;
    this._l7DdosProtection.internalValue = config.l7DdosProtection;
    this._malwareProtectionSettings.internalValue = config.malwareProtectionSettings;
    this._moreOption.internalValue = config.moreOption;
    this._originServerSubsetRuleList.internalValue = config.originServerSubsetRuleList;
    this._policyBasedChallenge.internalValue = config.policyBasedChallenge;
    this._protectedCookies.internalValue = config.protectedCookies;
    this._rateLimit.internalValue = config.rateLimit;
    this._ringHash.internalValue = config.ringHash;
    this._routes.internalValue = config.routes;
    this._sensitiveDataDisclosureRules.internalValue = config.sensitiveDataDisclosureRules;
    this._sensitiveDataPolicy.internalValue = config.sensitiveDataPolicy;
    this._slowDdosMitigation.internalValue = config.slowDdosMitigation;
    this._trustedClients.internalValue = config.trustedClients;
    this._userIdentification.internalValue = config.userIdentification;
    this._wafExclusionRules.internalValue = config.wafExclusionRules;
  }

  // ==========
  // ATTRIBUTES
  // ==========

  // add_location - computed: false, optional: true, required: false
  private _addLocation?: boolean | cdktf.IResolvable; 
  public get addLocation() {
    return this.getBooleanAttribute('add_location');
  }
  public set addLocation(value: boolean | cdktf.IResolvable) {
    this._addLocation = value;
  }
  public resetAddLocation() {
    this._addLocation = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get addLocationInput() {
    return this._addLocation;
  }

  // advertise_on_public_default_vip - computed: false, optional: true, required: false
  private _advertiseOnPublicDefaultVip?: boolean | cdktf.IResolvable; 
  public get advertiseOnPublicDefaultVip() {
    return this.getBooleanAttribute('advertise_on_public_default_vip');
  }
  public set advertiseOnPublicDefaultVip(value: boolean | cdktf.IResolvable) {
    this._advertiseOnPublicDefaultVip = value;
  }
  public resetAdvertiseOnPublicDefaultVip() {
    this._advertiseOnPublicDefaultVip = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get advertiseOnPublicDefaultVipInput() {
    return this._advertiseOnPublicDefaultVip;
  }

  // annotations - computed: false, optional: true, required: false
  private _annotations?: { [key: string]: string }; 
  public get annotations() {
    return this.getStringMapAttribute('annotations');
  }
  public set annotations(value: { [key: string]: string }) {
    this._annotations = value;
  }
  public resetAnnotations() {
    this._annotations = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get annotationsInput() {
    return this._annotations;
  }

  // cname - computed: true, optional: false, required: false
  public get cname() {
    return this.getStringAttribute('cname');
  }

  // default_sensitive_data_policy - computed: false, optional: true, required: false
  private _defaultSensitiveDataPolicy?: boolean | cdktf.IResolvable; 
  public get defaultSensitiveDataPolicy() {
    return this.getBooleanAttribute('default_sensitive_data_policy');
  }
  public set defaultSensitiveDataPolicy(value: boolean | cdktf.IResolvable) {
    this._defaultSensitiveDataPolicy = value;
  }
  public resetDefaultSensitiveDataPolicy() {
    this._defaultSensitiveDataPolicy = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get defaultSensitiveDataPolicyInput() {
    return this._defaultSensitiveDataPolicy;
  }

  // description - computed: false, optional: true, required: false
  private _description?: string; 
  public get description() {
    return this.getStringAttribute('description');
  }
  public set description(value: string) {
    this._description = value;
  }
  public resetDescription() {
    this._description = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get descriptionInput() {
    return this._description;
  }

  // disable - computed: false, optional: true, required: false
  private _disable?: boolean | cdktf.IResolvable; 
  public get disable() {
    return this.getBooleanAttribute('disable');
  }
  public set disable(value: boolean | cdktf.IResolvable) {
    this._disable = value;
  }
  public resetDisable() {
    this._disable = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get disableInput() {
    return this._disable;
  }

  // disable_api_definition - computed: false, optional: true, required: false
  private _disableApiDefinition?: boolean | cdktf.IResolvable; 
  public get disableApiDefinition() {
    return this.getBooleanAttribute('disable_api_definition');
  }
  public set disableApiDefinition(value: boolean | cdktf.IResolvable) {
    this._disableApiDefinition = value;
  }
  public resetDisableApiDefinition() {
    this._disableApiDefinition = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get disableApiDefinitionInput() {
    return this._disableApiDefinition;
  }

  // disable_api_discovery - computed: false, optional: true, required: false
  private _disableApiDiscovery?: boolean | cdktf.IResolvable; 
  public get disableApiDiscovery() {
    return this.getBooleanAttribute('disable_api_discovery');
  }
  public set disableApiDiscovery(value: boolean | cdktf.IResolvable) {
    this._disableApiDiscovery = value;
  }
  public resetDisableApiDiscovery() {
    this._disableApiDiscovery = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get disableApiDiscoveryInput() {
    return this._disableApiDiscovery;
  }

  // disable_api_testing - computed: false, optional: true, required: false
  private _disableApiTesting?: boolean | cdktf.IResolvable; 
  public get disableApiTesting() {
    return this.getBooleanAttribute('disable_api_testing');
  }
  public set disableApiTesting(value: boolean | cdktf.IResolvable) {
    this._disableApiTesting = value;
  }
  public resetDisableApiTesting() {
    this._disableApiTesting = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get disableApiTestingInput() {
    return this._disableApiTesting;
  }

  // disable_bot_defense - computed: false, optional: true, required: false
  private _disableBotDefense?: boolean | cdktf.IResolvable; 
  public get disableBotDefense() {
    return this.getBooleanAttribute('disable_bot_defense');
  }
  public set disableBotDefense(value: boolean | cdktf.IResolvable) {
    this._disableBotDefense = value;
  }
  public resetDisableBotDefense() {
    this._disableBotDefense = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get disableBotDefenseInput() {
    return this._disableBotDefense;
  }

  // disable_client_side_defense - computed: false, optional: true, required: false
  private _disableClientSideDefense?: boolean | cdktf.IResolvable; 
  public get disableClientSideDefense() {
    return this.getBooleanAttribute('disable_client_side_defense');
  }
  public set disableClientSideDefense(value: boolean | cdktf.IResolvable) {
    this._disableClientSideDefense = value;
  }
  public resetDisableClientSideDefense() {
    this._disableClientSideDefense = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get disableClientSideDefenseInput() {
    return this._disableClientSideDefense;
  }

  // disable_ip_reputation - computed: false, optional: true, required: false
  private _disableIpReputation?: boolean | cdktf.IResolvable; 
  public get disableIpReputation() {
    return this.getBooleanAttribute('disable_ip_reputation');
  }
  public set disableIpReputation(value: boolean | cdktf.IResolvable) {
    this._disableIpReputation = value;
  }
  public resetDisableIpReputation() {
    this._disableIpReputation = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get disableIpReputationInput() {
    return this._disableIpReputation;
  }

  // disable_malicious_user_detection - computed: false, optional: true, required: false
  private _disableMaliciousUserDetection?: boolean | cdktf.IResolvable; 
  public get disableMaliciousUserDetection() {
    return this.getBooleanAttribute('disable_malicious_user_detection');
  }
  public set disableMaliciousUserDetection(value: boolean | cdktf.IResolvable) {
    this._disableMaliciousUserDetection = value;
  }
  public resetDisableMaliciousUserDetection() {
    this._disableMaliciousUserDetection = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get disableMaliciousUserDetectionInput() {
    return this._disableMaliciousUserDetection;
  }

  // disable_malware_protection - computed: false, optional: true, required: false
  private _disableMalwareProtection?: boolean | cdktf.IResolvable; 
  public get disableMalwareProtection() {
    return this.getBooleanAttribute('disable_malware_protection');
  }
  public set disableMalwareProtection(value: boolean | cdktf.IResolvable) {
    this._disableMalwareProtection = value;
  }
  public resetDisableMalwareProtection() {
    this._disableMalwareProtection = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get disableMalwareProtectionInput() {
    return this._disableMalwareProtection;
  }

  // disable_rate_limit - computed: false, optional: true, required: false
  private _disableRateLimit?: boolean | cdktf.IResolvable; 
  public get disableRateLimit() {
    return this.getBooleanAttribute('disable_rate_limit');
  }
  public set disableRateLimit(value: boolean | cdktf.IResolvable) {
    this._disableRateLimit = value;
  }
  public resetDisableRateLimit() {
    this._disableRateLimit = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get disableRateLimitInput() {
    return this._disableRateLimit;
  }

  // disable_threat_mesh - computed: false, optional: true, required: false
  private _disableThreatMesh?: boolean | cdktf.IResolvable; 
  public get disableThreatMesh() {
    return this.getBooleanAttribute('disable_threat_mesh');
  }
  public set disableThreatMesh(value: boolean | cdktf.IResolvable) {
    this._disableThreatMesh = value;
  }
  public resetDisableThreatMesh() {
    this._disableThreatMesh = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get disableThreatMeshInput() {
    return this._disableThreatMesh;
  }

  // disable_trust_client_ip_headers - computed: false, optional: true, required: false
  private _disableTrustClientIpHeaders?: boolean | cdktf.IResolvable; 
  public get disableTrustClientIpHeaders() {
    return this.getBooleanAttribute('disable_trust_client_ip_headers');
  }
  public set disableTrustClientIpHeaders(value: boolean | cdktf.IResolvable) {
    this._disableTrustClientIpHeaders = value;
  }
  public resetDisableTrustClientIpHeaders() {
    this._disableTrustClientIpHeaders = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get disableTrustClientIpHeadersInput() {
    return this._disableTrustClientIpHeaders;
  }

  // disable_waf - computed: false, optional: true, required: false
  private _disableWaf?: boolean | cdktf.IResolvable; 
  public get disableWaf() {
    return this.getBooleanAttribute('disable_waf');
  }
  public set disableWaf(value: boolean | cdktf.IResolvable) {
    this._disableWaf = value;
  }
  public resetDisableWaf() {
    this._disableWaf = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get disableWafInput() {
    return this._disableWaf;
  }

  // do_not_advertise - computed: false, optional: true, required: false
  private _doNotAdvertise?: boolean | cdktf.IResolvable; 
  public get doNotAdvertise() {
    return this.getBooleanAttribute('do_not_advertise');
  }
  public set doNotAdvertise(value: boolean | cdktf.IResolvable) {
    this._doNotAdvertise = value;
  }
  public resetDoNotAdvertise() {
    this._doNotAdvertise = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get doNotAdvertiseInput() {
    return this._doNotAdvertise;
  }

  // domains - computed: false, optional: false, required: true
  private _domains?: string[]; 
  public get domains() {
    return this.getListAttribute('domains');
  }
  public set domains(value: string[]) {
    this._domains = value;
  }
  // Temporarily expose input value. Use with caution.
  public get domainsInput() {
    return this._domains;
  }

  // enable_malicious_user_detection - computed: false, optional: true, required: false
  private _enableMaliciousUserDetection?: boolean | cdktf.IResolvable; 
  public get enableMaliciousUserDetection() {
    return this.getBooleanAttribute('enable_malicious_user_detection');
  }
  public set enableMaliciousUserDetection(value: boolean | cdktf.IResolvable) {
    this._enableMaliciousUserDetection = value;
  }
  public resetEnableMaliciousUserDetection() {
    this._enableMaliciousUserDetection = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get enableMaliciousUserDetectionInput() {
    return this._enableMaliciousUserDetection;
  }

  // enable_threat_mesh - computed: false, optional: true, required: false
  private _enableThreatMesh?: boolean | cdktf.IResolvable; 
  public get enableThreatMesh() {
    return this.getBooleanAttribute('enable_threat_mesh');
  }
  public set enableThreatMesh(value: boolean | cdktf.IResolvable) {
    this._enableThreatMesh = value;
  }
  public resetEnableThreatMesh() {
    this._enableThreatMesh = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get enableThreatMeshInput() {
    return this._enableThreatMesh;
  }

  // id - computed: true, optional: true, required: false
  private _id?: string; 
  public get id() {
    return this.getStringAttribute('id');
  }
  public set id(value: string) {
    this._id = value;
  }
  public resetId() {
    this._id = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get idInput() {
    return this._id;
  }

  // labels - computed: false, optional: true, required: false
  private _labels?: { [key: string]: string }; 
  public get labels() {
    return this.getStringMapAttribute('labels');
  }
  public set labels(value: { [key: string]: string }) {
    this._labels = value;
  }
  public resetLabels() {
    this._labels = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get labelsInput() {
    return this._labels;
  }

  // least_active - computed: false, optional: true, required: false
  private _leastActive?: boolean | cdktf.IResolvable; 
  public get leastActive() {
    return this.getBooleanAttribute('least_active');
  }
  public set leastActive(value: boolean | cdktf.IResolvable) {
    this._leastActive = value;
  }
  public resetLeastActive() {
    this._leastActive = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get leastActiveInput() {
    return this._leastActive;
  }

  // name - computed: false, optional: false, required: true
  private _name?: string; 
  public get name() {
    return this.getStringAttribute('name');
  }
  public set name(value: string) {
    this._name = value;
  }
  // Temporarily expose input value. Use with caution.
  public get nameInput() {
    return this._name;
  }

  // namespace - computed: false, optional: false, required: true
  private _namespace?: string; 
  public get namespace() {
    return this.getStringAttribute('namespace');
  }
  public set namespace(value: string) {
    this._namespace = value;
  }
  // Temporarily expose input value. Use with caution.
  public get namespaceInput() {
    return this._namespace;
  }

  // no_challenge - computed: false, optional: true, required: false
  private _noChallenge?: boolean | cdktf.IResolvable; 
  public get noChallenge() {
    return this.getBooleanAttribute('no_challenge');
  }
  public set noChallenge(value: boolean | cdktf.IResolvable) {
    this._noChallenge = value;
  }
  public resetNoChallenge() {
    this._noChallenge = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get noChallengeInput() {
    return this._noChallenge;
  }

  // no_service_policies - computed: false, optional: true, required: false
  private _noServicePolicies?: boolean | cdktf.IResolvable; 
  public get noServicePolicies() {
    return this.getBooleanAttribute('no_service_policies');
  }
  public set noServicePolicies(value: boolean | cdktf.IResolvable) {
    this._noServicePolicies = value;
  }
  public resetNoServicePolicies() {
    this._noServicePolicies = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get noServicePoliciesInput() {
    return this._noServicePolicies;
  }

  // random - computed: false, optional: true, required: false
  private _random?: boolean | cdktf.IResolvable; 
  public get random() {
    return this.getBooleanAttribute('random');
  }
  public set random(value: boolean | cdktf.IResolvable) {
    this._random = value;
  }
  public resetRandom() {
    this._random = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get randomInput() {
    return this._random;
  }

  // round_robin - computed: false, optional: true, required: false
  private _roundRobin?: boolean | cdktf.IResolvable; 
  public get roundRobin() {
    return this.getBooleanAttribute('round_robin');
  }
  public set roundRobin(value: boolean | cdktf.IResolvable) {
    this._roundRobin = value;
  }
  public resetRoundRobin() {
    this._roundRobin = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get roundRobinInput() {
    return this._roundRobin;
  }

  // service_policies_from_namespace - computed: false, optional: true, required: false
  private _servicePoliciesFromNamespace?: boolean | cdktf.IResolvable; 
  public get servicePoliciesFromNamespace() {
    return this.getBooleanAttribute('service_policies_from_namespace');
  }
  public set servicePoliciesFromNamespace(value: boolean | cdktf.IResolvable) {
    this._servicePoliciesFromNamespace = value;
  }
  public resetServicePoliciesFromNamespace() {
    this._servicePoliciesFromNamespace = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get servicePoliciesFromNamespaceInput() {
    return this._servicePoliciesFromNamespace;
  }

  // source_ip_stickiness - computed: false, optional: true, required: false
  private _sourceIpStickiness?: boolean | cdktf.IResolvable; 
  public get sourceIpStickiness() {
    return this.getBooleanAttribute('source_ip_stickiness');
  }
  public set sourceIpStickiness(value: boolean | cdktf.IResolvable) {
    this._sourceIpStickiness = value;
  }
  public resetSourceIpStickiness() {
    this._sourceIpStickiness = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get sourceIpStickinessInput() {
    return this._sourceIpStickiness;
  }

  // system_default_timeouts - computed: false, optional: true, required: false
  private _systemDefaultTimeouts?: boolean | cdktf.IResolvable; 
  public get systemDefaultTimeouts() {
    return this.getBooleanAttribute('system_default_timeouts');
  }
  public set systemDefaultTimeouts(value: boolean | cdktf.IResolvable) {
    this._systemDefaultTimeouts = value;
  }
  public resetSystemDefaultTimeouts() {
    this._systemDefaultTimeouts = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get systemDefaultTimeoutsInput() {
    return this._systemDefaultTimeouts;
  }

  // user_id_client_ip - computed: false, optional: true, required: false
  private _userIdClientIp?: boolean | cdktf.IResolvable; 
  public get userIdClientIp() {
    return this.getBooleanAttribute('user_id_client_ip');
  }
  public set userIdClientIp(value: boolean | cdktf.IResolvable) {
    this._userIdClientIp = value;
  }
  public resetUserIdClientIp() {
    this._userIdClientIp = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get userIdClientIpInput() {
    return this._userIdClientIp;
  }

  // active_service_policies - computed: false, optional: true, required: false
  private _activeServicePolicies = new HttpLoadbalancerActiveServicePoliciesOutputReference(this, "active_service_policies");
  public get activeServicePolicies() {
    return this._activeServicePolicies;
  }
  public putActiveServicePolicies(value: HttpLoadbalancerActiveServicePolicies) {
    this._activeServicePolicies.internalValue = value;
  }
  public resetActiveServicePolicies() {
    this._activeServicePolicies.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get activeServicePoliciesInput() {
    return this._activeServicePolicies.internalValue;
  }

  // advertise_custom - computed: false, optional: true, required: false
  private _advertiseCustom = new HttpLoadbalancerAdvertiseCustomOutputReference(this, "advertise_custom");
  public get advertiseCustom() {
    return this._advertiseCustom;
  }
  public putAdvertiseCustom(value: HttpLoadbalancerAdvertiseCustom) {
    this._advertiseCustom.internalValue = value;
  }
  public resetAdvertiseCustom() {
    this._advertiseCustom.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get advertiseCustomInput() {
    return this._advertiseCustom.internalValue;
  }

  // advertise_on_public - computed: false, optional: true, required: false
  private _advertiseOnPublic = new HttpLoadbalancerAdvertiseOnPublicOutputReference(this, "advertise_on_public");
  public get advertiseOnPublic() {
    return this._advertiseOnPublic;
  }
  public putAdvertiseOnPublic(value: HttpLoadbalancerAdvertiseOnPublic) {
    this._advertiseOnPublic.internalValue = value;
  }
  public resetAdvertiseOnPublic() {
    this._advertiseOnPublic.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get advertiseOnPublicInput() {
    return this._advertiseOnPublic.internalValue;
  }

  // api_protection_rules - computed: false, optional: true, required: false
  private _apiProtectionRules = new HttpLoadbalancerApiProtectionRulesOutputReference(this, "api_protection_rules");
  public get apiProtectionRules() {
    return this._apiProtectionRules;
  }
  public putApiProtectionRules(value: HttpLoadbalancerApiProtectionRules) {
    this._apiProtectionRules.internalValue = value;
  }
  public resetApiProtectionRules() {
    this._apiProtectionRules.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get apiProtectionRulesInput() {
    return this._apiProtectionRules.internalValue;
  }

  // api_rate_limit - computed: false, optional: true, required: false
  private _apiRateLimit = new HttpLoadbalancerApiRateLimitOutputReference(this, "api_rate_limit");
  public get apiRateLimit() {
    return this._apiRateLimit;
  }
  public putApiRateLimit(value: HttpLoadbalancerApiRateLimit) {
    this._apiRateLimit.internalValue = value;
  }
  public resetApiRateLimit() {
    this._apiRateLimit.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get apiRateLimitInput() {
    return this._apiRateLimit.internalValue;
  }

  // api_specification - computed: false, optional: true, required: false
  private _apiSpecification = new HttpLoadbalancerApiSpecificationOutputReference(this, "api_specification");
  public get apiSpecification() {
    return this._apiSpecification;
  }
  public putApiSpecification(value: HttpLoadbalancerApiSpecification) {
    this._apiSpecification.internalValue = value;
  }
  public resetApiSpecification() {
    this._apiSpecification.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get apiSpecificationInput() {
    return this._apiSpecification.internalValue;
  }

  // api_testing - computed: false, optional: true, required: false
  private _apiTesting = new HttpLoadbalancerApiTestingOutputReference(this, "api_testing");
  public get apiTesting() {
    return this._apiTesting;
  }
  public putApiTesting(value: HttpLoadbalancerApiTesting) {
    this._apiTesting.internalValue = value;
  }
  public resetApiTesting() {
    this._apiTesting.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get apiTestingInput() {
    return this._apiTesting.internalValue;
  }

  // app_firewall - computed: false, optional: true, required: false
  private _appFirewall = new HttpLoadbalancerAppFirewallOutputReference(this, "app_firewall");
  public get appFirewall() {
    return this._appFirewall;
  }
  public putAppFirewall(value: HttpLoadbalancerAppFirewall) {
    this._appFirewall.internalValue = value;
  }
  public resetAppFirewall() {
    this._appFirewall.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get appFirewallInput() {
    return this._appFirewall.internalValue;
  }

  // blocked_clients - computed: false, optional: true, required: false
  private _blockedClients = new HttpLoadbalancerBlockedClientsList(this, "blocked_clients", false);
  public get blockedClients() {
    return this._blockedClients;
  }
  public putBlockedClients(value: HttpLoadbalancerBlockedClients[] | cdktf.IResolvable) {
    this._blockedClients.internalValue = value;
  }
  public resetBlockedClients() {
    this._blockedClients.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get blockedClientsInput() {
    return this._blockedClients.internalValue;
  }

  // bot_defense - computed: false, optional: true, required: false
  private _botDefense = new HttpLoadbalancerBotDefenseOutputReference(this, "bot_defense");
  public get botDefense() {
    return this._botDefense;
  }
  public putBotDefense(value: HttpLoadbalancerBotDefense) {
    this._botDefense.internalValue = value;
  }
  public resetBotDefense() {
    this._botDefense.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get botDefenseInput() {
    return this._botDefense.internalValue;
  }

  // captcha_challenge - computed: false, optional: true, required: false
  private _captchaChallenge = new HttpLoadbalancerCaptchaChallengeOutputReference(this, "captcha_challenge");
  public get captchaChallenge() {
    return this._captchaChallenge;
  }
  public putCaptchaChallenge(value: HttpLoadbalancerCaptchaChallenge) {
    this._captchaChallenge.internalValue = value;
  }
  public resetCaptchaChallenge() {
    this._captchaChallenge.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get captchaChallengeInput() {
    return this._captchaChallenge.internalValue;
  }

  // client_side_defense - computed: false, optional: true, required: false
  private _clientSideDefense = new HttpLoadbalancerClientSideDefenseOutputReference(this, "client_side_defense");
  public get clientSideDefense() {
    return this._clientSideDefense;
  }
  public putClientSideDefense(value: HttpLoadbalancerClientSideDefense) {
    this._clientSideDefense.internalValue = value;
  }
  public resetClientSideDefense() {
    this._clientSideDefense.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get clientSideDefenseInput() {
    return this._clientSideDefense.internalValue;
  }

  // cookie_stickiness - computed: false, optional: true, required: false
  private _cookieStickiness = new HttpLoadbalancerCookieStickinessOutputReference(this, "cookie_stickiness");
  public get cookieStickiness() {
    return this._cookieStickiness;
  }
  public putCookieStickiness(value: HttpLoadbalancerCookieStickiness) {
    this._cookieStickiness.internalValue = value;
  }
  public resetCookieStickiness() {
    this._cookieStickiness.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get cookieStickinessInput() {
    return this._cookieStickiness.internalValue;
  }

  // cors_policy - computed: false, optional: true, required: false
  private _corsPolicy = new HttpLoadbalancerCorsPolicyOutputReference(this, "cors_policy");
  public get corsPolicy() {
    return this._corsPolicy;
  }
  public putCorsPolicy(value: HttpLoadbalancerCorsPolicy) {
    this._corsPolicy.internalValue = value;
  }
  public resetCorsPolicy() {
    this._corsPolicy.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get corsPolicyInput() {
    return this._corsPolicy.internalValue;
  }

  // csrf_policy - computed: false, optional: true, required: false
  private _csrfPolicy = new HttpLoadbalancerCsrfPolicyOutputReference(this, "csrf_policy");
  public get csrfPolicy() {
    return this._csrfPolicy;
  }
  public putCsrfPolicy(value: HttpLoadbalancerCsrfPolicy) {
    this._csrfPolicy.internalValue = value;
  }
  public resetCsrfPolicy() {
    this._csrfPolicy.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get csrfPolicyInput() {
    return this._csrfPolicy.internalValue;
  }

  // data_guard_rules - computed: false, optional: true, required: false
  private _dataGuardRules = new HttpLoadbalancerDataGuardRulesList(this, "data_guard_rules", false);
  public get dataGuardRules() {
    return this._dataGuardRules;
  }
  public putDataGuardRules(value: HttpLoadbalancerDataGuardRules[] | cdktf.IResolvable) {
    this._dataGuardRules.internalValue = value;
  }
  public resetDataGuardRules() {
    this._dataGuardRules.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get dataGuardRulesInput() {
    return this._dataGuardRules.internalValue;
  }

  // ddos_mitigation_rules - computed: false, optional: true, required: false
  private _ddosMitigationRules = new HttpLoadbalancerDdosMitigationRulesList(this, "ddos_mitigation_rules", false);
  public get ddosMitigationRules() {
    return this._ddosMitigationRules;
  }
  public putDdosMitigationRules(value: HttpLoadbalancerDdosMitigationRules[] | cdktf.IResolvable) {
    this._ddosMitigationRules.internalValue = value;
  }
  public resetDdosMitigationRules() {
    this._ddosMitigationRules.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get ddosMitigationRulesInput() {
    return this._ddosMitigationRules.internalValue;
  }

  // default_route_pools - computed: false, optional: true, required: false
  private _defaultRoutePools = new HttpLoadbalancerDefaultRoutePoolsList(this, "default_route_pools", false);
  public get defaultRoutePools() {
    return this._defaultRoutePools;
  }
  public putDefaultRoutePools(value: HttpLoadbalancerDefaultRoutePools[] | cdktf.IResolvable) {
    this._defaultRoutePools.internalValue = value;
  }
  public resetDefaultRoutePools() {
    this._defaultRoutePools.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get defaultRoutePoolsInput() {
    return this._defaultRoutePools.internalValue;
  }

  // enable_api_discovery - computed: false, optional: true, required: false
  private _enableApiDiscovery = new HttpLoadbalancerEnableApiDiscoveryOutputReference(this, "enable_api_discovery");
  public get enableApiDiscovery() {
    return this._enableApiDiscovery;
  }
  public putEnableApiDiscovery(value: HttpLoadbalancerEnableApiDiscovery) {
    this._enableApiDiscovery.internalValue = value;
  }
  public resetEnableApiDiscovery() {
    this._enableApiDiscovery.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get enableApiDiscoveryInput() {
    return this._enableApiDiscovery.internalValue;
  }

  // enable_challenge - computed: false, optional: true, required: false
  private _enableChallenge = new HttpLoadbalancerEnableChallengeOutputReference(this, "enable_challenge");
  public get enableChallenge() {
    return this._enableChallenge;
  }
  public putEnableChallenge(value: HttpLoadbalancerEnableChallenge) {
    this._enableChallenge.internalValue = value;
  }
  public resetEnableChallenge() {
    this._enableChallenge.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get enableChallengeInput() {
    return this._enableChallenge.internalValue;
  }

  // enable_ip_reputation - computed: false, optional: true, required: false
  private _enableIpReputation = new HttpLoadbalancerEnableIpReputationOutputReference(this, "enable_ip_reputation");
  public get enableIpReputation() {
    return this._enableIpReputation;
  }
  public putEnableIpReputation(value: HttpLoadbalancerEnableIpReputation) {
    this._enableIpReputation.internalValue = value;
  }
  public resetEnableIpReputation() {
    this._enableIpReputation.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get enableIpReputationInput() {
    return this._enableIpReputation.internalValue;
  }

  // enable_trust_client_ip_headers - computed: false, optional: true, required: false
  private _enableTrustClientIpHeaders = new HttpLoadbalancerEnableTrustClientIpHeadersOutputReference(this, "enable_trust_client_ip_headers");
  public get enableTrustClientIpHeaders() {
    return this._enableTrustClientIpHeaders;
  }
  public putEnableTrustClientIpHeaders(value: HttpLoadbalancerEnableTrustClientIpHeaders) {
    this._enableTrustClientIpHeaders.internalValue = value;
  }
  public resetEnableTrustClientIpHeaders() {
    this._enableTrustClientIpHeaders.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get enableTrustClientIpHeadersInput() {
    return this._enableTrustClientIpHeaders.internalValue;
  }

  // graphql_rules - computed: false, optional: true, required: false
  private _graphqlRules = new HttpLoadbalancerGraphqlRulesList(this, "graphql_rules", false);
  public get graphqlRules() {
    return this._graphqlRules;
  }
  public putGraphqlRules(value: HttpLoadbalancerGraphqlRules[] | cdktf.IResolvable) {
    this._graphqlRules.internalValue = value;
  }
  public resetGraphqlRules() {
    this._graphqlRules.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get graphqlRulesInput() {
    return this._graphqlRules.internalValue;
  }

  // http - computed: false, optional: true, required: false
  private _http = new HttpLoadbalancerHttpOutputReference(this, "http");
  public get http() {
    return this._http;
  }
  public putHttp(value: HttpLoadbalancerHttp) {
    this._http.internalValue = value;
  }
  public resetHttp() {
    this._http.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get httpInput() {
    return this._http.internalValue;
  }

  // https - computed: false, optional: true, required: false
  private _https = new HttpLoadbalancerHttpsOutputReference(this, "https");
  public get https() {
    return this._https;
  }
  public putHttps(value: HttpLoadbalancerHttps) {
    this._https.internalValue = value;
  }
  public resetHttps() {
    this._https.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get httpsInput() {
    return this._https.internalValue;
  }

  // https_auto_cert - computed: false, optional: true, required: false
  private _httpsAutoCert = new HttpLoadbalancerHttpsAutoCertOutputReference(this, "https_auto_cert");
  public get httpsAutoCert() {
    return this._httpsAutoCert;
  }
  public putHttpsAutoCert(value: HttpLoadbalancerHttpsAutoCert) {
    this._httpsAutoCert.internalValue = value;
  }
  public resetHttpsAutoCert() {
    this._httpsAutoCert.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get httpsAutoCertInput() {
    return this._httpsAutoCert.internalValue;
  }

  // js_challenge - computed: false, optional: true, required: false
  private _jsChallenge = new HttpLoadbalancerJsChallengeOutputReference(this, "js_challenge");
  public get jsChallenge() {
    return this._jsChallenge;
  }
  public putJsChallenge(value: HttpLoadbalancerJsChallenge) {
    this._jsChallenge.internalValue = value;
  }
  public resetJsChallenge() {
    this._jsChallenge.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get jsChallengeInput() {
    return this._jsChallenge.internalValue;
  }

  // jwt_validation - computed: false, optional: true, required: false
  private _jwtValidation = new HttpLoadbalancerJwtValidationOutputReference(this, "jwt_validation");
  public get jwtValidation() {
    return this._jwtValidation;
  }
  public putJwtValidation(value: HttpLoadbalancerJwtValidation) {
    this._jwtValidation.internalValue = value;
  }
  public resetJwtValidation() {
    this._jwtValidation.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get jwtValidationInput() {
    return this._jwtValidation.internalValue;
  }

  // l7_ddos_protection - computed: false, optional: true, required: false
  private _l7DdosProtection = new HttpLoadbalancerL7DdosProtectionOutputReference(this, "l7_ddos_protection");
  public get l7DdosProtection() {
    return this._l7DdosProtection;
  }
  public putL7DdosProtection(value: HttpLoadbalancerL7DdosProtection) {
    this._l7DdosProtection.internalValue = value;
  }
  public resetL7DdosProtection() {
    this._l7DdosProtection.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get l7DdosProtectionInput() {
    return this._l7DdosProtection.internalValue;
  }

  // malware_protection_settings - computed: false, optional: true, required: false
  private _malwareProtectionSettings = new HttpLoadbalancerMalwareProtectionSettingsOutputReference(this, "malware_protection_settings");
  public get malwareProtectionSettings() {
    return this._malwareProtectionSettings;
  }
  public putMalwareProtectionSettings(value: HttpLoadbalancerMalwareProtectionSettings) {
    this._malwareProtectionSettings.internalValue = value;
  }
  public resetMalwareProtectionSettings() {
    this._malwareProtectionSettings.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get malwareProtectionSettingsInput() {
    return this._malwareProtectionSettings.internalValue;
  }

  // more_option - computed: false, optional: true, required: false
  private _moreOption = new HttpLoadbalancerMoreOptionOutputReference(this, "more_option");
  public get moreOption() {
    return this._moreOption;
  }
  public putMoreOption(value: HttpLoadbalancerMoreOption) {
    this._moreOption.internalValue = value;
  }
  public resetMoreOption() {
    this._moreOption.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get moreOptionInput() {
    return this._moreOption.internalValue;
  }

  // origin_server_subset_rule_list - computed: false, optional: true, required: false
  private _originServerSubsetRuleList = new HttpLoadbalancerOriginServerSubsetRuleListStructOutputReference(this, "origin_server_subset_rule_list");
  public get originServerSubsetRuleList() {
    return this._originServerSubsetRuleList;
  }
  public putOriginServerSubsetRuleList(value: HttpLoadbalancerOriginServerSubsetRuleListStruct) {
    this._originServerSubsetRuleList.internalValue = value;
  }
  public resetOriginServerSubsetRuleList() {
    this._originServerSubsetRuleList.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get originServerSubsetRuleListInput() {
    return this._originServerSubsetRuleList.internalValue;
  }

  // policy_based_challenge - computed: false, optional: true, required: false
  private _policyBasedChallenge = new HttpLoadbalancerPolicyBasedChallengeOutputReference(this, "policy_based_challenge");
  public get policyBasedChallenge() {
    return this._policyBasedChallenge;
  }
  public putPolicyBasedChallenge(value: HttpLoadbalancerPolicyBasedChallenge) {
    this._policyBasedChallenge.internalValue = value;
  }
  public resetPolicyBasedChallenge() {
    this._policyBasedChallenge.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get policyBasedChallengeInput() {
    return this._policyBasedChallenge.internalValue;
  }

  // protected_cookies - computed: false, optional: true, required: false
  private _protectedCookies = new HttpLoadbalancerProtectedCookiesList(this, "protected_cookies", false);
  public get protectedCookies() {
    return this._protectedCookies;
  }
  public putProtectedCookies(value: HttpLoadbalancerProtectedCookies[] | cdktf.IResolvable) {
    this._protectedCookies.internalValue = value;
  }
  public resetProtectedCookies() {
    this._protectedCookies.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get protectedCookiesInput() {
    return this._protectedCookies.internalValue;
  }

  // rate_limit - computed: false, optional: true, required: false
  private _rateLimit = new HttpLoadbalancerRateLimitOutputReference(this, "rate_limit");
  public get rateLimit() {
    return this._rateLimit;
  }
  public putRateLimit(value: HttpLoadbalancerRateLimit) {
    this._rateLimit.internalValue = value;
  }
  public resetRateLimit() {
    this._rateLimit.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get rateLimitInput() {
    return this._rateLimit.internalValue;
  }

  // ring_hash - computed: false, optional: true, required: false
  private _ringHash = new HttpLoadbalancerRingHashOutputReference(this, "ring_hash");
  public get ringHash() {
    return this._ringHash;
  }
  public putRingHash(value: HttpLoadbalancerRingHash) {
    this._ringHash.internalValue = value;
  }
  public resetRingHash() {
    this._ringHash.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get ringHashInput() {
    return this._ringHash.internalValue;
  }

  // routes - computed: false, optional: true, required: false
  private _routes = new HttpLoadbalancerRoutesList(this, "routes", false);
  public get routes() {
    return this._routes;
  }
  public putRoutes(value: HttpLoadbalancerRoutes[] | cdktf.IResolvable) {
    this._routes.internalValue = value;
  }
  public resetRoutes() {
    this._routes.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get routesInput() {
    return this._routes.internalValue;
  }

  // sensitive_data_disclosure_rules - computed: false, optional: true, required: false
  private _sensitiveDataDisclosureRules = new HttpLoadbalancerSensitiveDataDisclosureRulesOutputReference(this, "sensitive_data_disclosure_rules");
  public get sensitiveDataDisclosureRules() {
    return this._sensitiveDataDisclosureRules;
  }
  public putSensitiveDataDisclosureRules(value: HttpLoadbalancerSensitiveDataDisclosureRules) {
    this._sensitiveDataDisclosureRules.internalValue = value;
  }
  public resetSensitiveDataDisclosureRules() {
    this._sensitiveDataDisclosureRules.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get sensitiveDataDisclosureRulesInput() {
    return this._sensitiveDataDisclosureRules.internalValue;
  }

  // sensitive_data_policy - computed: false, optional: true, required: false
  private _sensitiveDataPolicy = new HttpLoadbalancerSensitiveDataPolicyOutputReference(this, "sensitive_data_policy");
  public get sensitiveDataPolicy() {
    return this._sensitiveDataPolicy;
  }
  public putSensitiveDataPolicy(value: HttpLoadbalancerSensitiveDataPolicy) {
    this._sensitiveDataPolicy.internalValue = value;
  }
  public resetSensitiveDataPolicy() {
    this._sensitiveDataPolicy.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get sensitiveDataPolicyInput() {
    return this._sensitiveDataPolicy.internalValue;
  }

  // slow_ddos_mitigation - computed: false, optional: true, required: false
  private _slowDdosMitigation = new HttpLoadbalancerSlowDdosMitigationOutputReference(this, "slow_ddos_mitigation");
  public get slowDdosMitigation() {
    return this._slowDdosMitigation;
  }
  public putSlowDdosMitigation(value: HttpLoadbalancerSlowDdosMitigation) {
    this._slowDdosMitigation.internalValue = value;
  }
  public resetSlowDdosMitigation() {
    this._slowDdosMitigation.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get slowDdosMitigationInput() {
    return this._slowDdosMitigation.internalValue;
  }

  // trusted_clients - computed: false, optional: true, required: false
  private _trustedClients = new HttpLoadbalancerTrustedClientsList(this, "trusted_clients", false);
  public get trustedClients() {
    return this._trustedClients;
  }
  public putTrustedClients(value: HttpLoadbalancerTrustedClients[] | cdktf.IResolvable) {
    this._trustedClients.internalValue = value;
  }
  public resetTrustedClients() {
    this._trustedClients.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get trustedClientsInput() {
    return this._trustedClients.internalValue;
  }

  // user_identification - computed: false, optional: true, required: false
  private _userIdentification = new HttpLoadbalancerUserIdentificationOutputReference(this, "user_identification");
  public get userIdentification() {
    return this._userIdentification;
  }
  public putUserIdentification(value: HttpLoadbalancerUserIdentification) {
    this._userIdentification.internalValue = value;
  }
  public resetUserIdentification() {
    this._userIdentification.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get userIdentificationInput() {
    return this._userIdentification.internalValue;
  }

  // waf_exclusion_rules - computed: false, optional: true, required: false
  private _wafExclusionRules = new HttpLoadbalancerWafExclusionRulesList(this, "waf_exclusion_rules", false);
  public get wafExclusionRules() {
    return this._wafExclusionRules;
  }
  public putWafExclusionRules(value: HttpLoadbalancerWafExclusionRules[] | cdktf.IResolvable) {
    this._wafExclusionRules.internalValue = value;
  }
  public resetWafExclusionRules() {
    this._wafExclusionRules.internalValue = undefined;
  }
  // Temporarily expose input value. Use with caution.
  public get wafExclusionRulesInput() {
    return this._wafExclusionRules.internalValue;
  }

  // =========
  // SYNTHESIS
  // =========

  protected synthesizeAttributes(): { [name: string]: any } {
    return {
      add_location: cdktf.booleanToTerraform(this._addLocation),
      advertise_on_public_default_vip: cdktf.booleanToTerraform(this._advertiseOnPublicDefaultVip),
      annotations: cdktf.hashMapper(cdktf.stringToTerraform)(this._annotations),
      default_sensitive_data_policy: cdktf.booleanToTerraform(this._defaultSensitiveDataPolicy),
      description: cdktf.stringToTerraform(this._description),
      disable: cdktf.booleanToTerraform(this._disable),
      disable_api_definition: cdktf.booleanToTerraform(this._disableApiDefinition),
      disable_api_discovery: cdktf.booleanToTerraform(this._disableApiDiscovery),
      disable_api_testing: cdktf.booleanToTerraform(this._disableApiTesting),
      disable_bot_defense: cdktf.booleanToTerraform(this._disableBotDefense),
      disable_client_side_defense: cdktf.booleanToTerraform(this._disableClientSideDefense),
      disable_ip_reputation: cdktf.booleanToTerraform(this._disableIpReputation),
      disable_malicious_user_detection: cdktf.booleanToTerraform(this._disableMaliciousUserDetection),
      disable_malware_protection: cdktf.booleanToTerraform(this._disableMalwareProtection),
      disable_rate_limit: cdktf.booleanToTerraform(this._disableRateLimit),
      disable_threat_mesh: cdktf.booleanToTerraform(this._disableThreatMesh),
      disable_trust_client_ip_headers: cdktf.booleanToTerraform(this._disableTrustClientIpHeaders),
      disable_waf: cdktf.booleanToTerraform(this._disableWaf),
      do_not_advertise: cdktf.booleanToTerraform(this._doNotAdvertise),
      domains: cdktf.listMapper(cdktf.stringToTerraform, false)(this._domains),
      enable_malicious_user_detection: cdktf.booleanToTerraform(this._enableMaliciousUserDetection),
      enable_threat_mesh: cdktf.booleanToTerraform(this._enableThreatMesh),
      id: cdktf.stringToTerraform(this._id),
      labels: cdktf.hashMapper(cdktf.stringToTerraform)(this._labels),
      least_active: cdktf.booleanToTerraform(this._leastActive),
      name: cdktf.stringToTerraform(this._name),
      namespace: cdktf.stringToTerraform(this._namespace),
      no_challenge: cdktf.booleanToTerraform(this._noChallenge),
      no_service_policies: cdktf.booleanToTerraform(this._noServicePolicies),
      random: cdktf.booleanToTerraform(this._random),
      round_robin: cdktf.booleanToTerraform(this._roundRobin),
      service_policies_from_namespace: cdktf.booleanToTerraform(this._servicePoliciesFromNamespace),
      source_ip_stickiness: cdktf.booleanToTerraform(this._sourceIpStickiness),
      system_default_timeouts: cdktf.booleanToTerraform(this._systemDefaultTimeouts),
      user_id_client_ip: cdktf.booleanToTerraform(this._userIdClientIp),
      active_service_policies: httpLoadbalancerActiveServicePoliciesToTerraform(this._activeServicePolicies.internalValue),
      advertise_custom: httpLoadbalancerAdvertiseCustomToTerraform(this._advertiseCustom.internalValue),
      advertise_on_public: httpLoadbalancerAdvertiseOnPublicToTerraform(this._advertiseOnPublic.internalValue),
      api_protection_rules: httpLoadbalancerApiProtectionRulesToTerraform(this._apiProtectionRules.internalValue),
      api_rate_limit: httpLoadbalancerApiRateLimitToTerraform(this._apiRateLimit.internalValue),
      api_specification: httpLoadbalancerApiSpecificationToTerraform(this._apiSpecification.internalValue),
      api_testing: httpLoadbalancerApiTestingToTerraform(this._apiTesting.internalValue),
      app_firewall: httpLoadbalancerAppFirewallToTerraform(this._appFirewall.internalValue),
      blocked_clients: cdktf.listMapper(httpLoadbalancerBlockedClientsToTerraform, true)(this._blockedClients.internalValue),
      bot_defense: httpLoadbalancerBotDefenseToTerraform(this._botDefense.internalValue),
      captcha_challenge: httpLoadbalancerCaptchaChallengeToTerraform(this._captchaChallenge.internalValue),
      client_side_defense: httpLoadbalancerClientSideDefenseToTerraform(this._clientSideDefense.internalValue),
      cookie_stickiness: httpLoadbalancerCookieStickinessToTerraform(this._cookieStickiness.internalValue),
      cors_policy: httpLoadbalancerCorsPolicyToTerraform(this._corsPolicy.internalValue),
      csrf_policy: httpLoadbalancerCsrfPolicyToTerraform(this._csrfPolicy.internalValue),
      data_guard_rules: cdktf.listMapper(httpLoadbalancerDataGuardRulesToTerraform, true)(this._dataGuardRules.internalValue),
      ddos_mitigation_rules: cdktf.listMapper(httpLoadbalancerDdosMitigationRulesToTerraform, true)(this._ddosMitigationRules.internalValue),
      default_route_pools: cdktf.listMapper(httpLoadbalancerDefaultRoutePoolsToTerraform, true)(this._defaultRoutePools.internalValue),
      enable_api_discovery: httpLoadbalancerEnableApiDiscoveryToTerraform(this._enableApiDiscovery.internalValue),
      enable_challenge: httpLoadbalancerEnableChallengeToTerraform(this._enableChallenge.internalValue),
      enable_ip_reputation: httpLoadbalancerEnableIpReputationToTerraform(this._enableIpReputation.internalValue),
      enable_trust_client_ip_headers: httpLoadbalancerEnableTrustClientIpHeadersToTerraform(this._enableTrustClientIpHeaders.internalValue),
      graphql_rules: cdktf.listMapper(httpLoadbalancerGraphqlRulesToTerraform, true)(this._graphqlRules.internalValue),
      http: httpLoadbalancerHttpToTerraform(this._http.internalValue),
      https: httpLoadbalancerHttpsToTerraform(this._https.internalValue),
      https_auto_cert: httpLoadbalancerHttpsAutoCertToTerraform(this._httpsAutoCert.internalValue),
      js_challenge: httpLoadbalancerJsChallengeToTerraform(this._jsChallenge.internalValue),
      jwt_validation: httpLoadbalancerJwtValidationToTerraform(this._jwtValidation.internalValue),
      l7_ddos_protection: httpLoadbalancerL7DdosProtectionToTerraform(this._l7DdosProtection.internalValue),
      malware_protection_settings: httpLoadbalancerMalwareProtectionSettingsToTerraform(this._malwareProtectionSettings.internalValue),
      more_option: httpLoadbalancerMoreOptionToTerraform(this._moreOption.internalValue),
      origin_server_subset_rule_list: httpLoadbalancerOriginServerSubsetRuleListStructToTerraform(this._originServerSubsetRuleList.internalValue),
      policy_based_challenge: httpLoadbalancerPolicyBasedChallengeToTerraform(this._policyBasedChallenge.internalValue),
      protected_cookies: cdktf.listMapper(httpLoadbalancerProtectedCookiesToTerraform, true)(this._protectedCookies.internalValue),
      rate_limit: httpLoadbalancerRateLimitToTerraform(this._rateLimit.internalValue),
      ring_hash: httpLoadbalancerRingHashToTerraform(this._ringHash.internalValue),
      routes: cdktf.listMapper(httpLoadbalancerRoutesToTerraform, true)(this._routes.internalValue),
      sensitive_data_disclosure_rules: httpLoadbalancerSensitiveDataDisclosureRulesToTerraform(this._sensitiveDataDisclosureRules.internalValue),
      sensitive_data_policy: httpLoadbalancerSensitiveDataPolicyToTerraform(this._sensitiveDataPolicy.internalValue),
      slow_ddos_mitigation: httpLoadbalancerSlowDdosMitigationToTerraform(this._slowDdosMitigation.internalValue),
      trusted_clients: cdktf.listMapper(httpLoadbalancerTrustedClientsToTerraform, true)(this._trustedClients.internalValue),
      user_identification: httpLoadbalancerUserIdentificationToTerraform(this._userIdentification.internalValue),
      waf_exclusion_rules: cdktf.listMapper(httpLoadbalancerWafExclusionRulesToTerraform, true)(this._wafExclusionRules.internalValue),
    };
  }

  protected synthesizeHclAttributes(): { [name: string]: any } {
    const attrs = {
      add_location: {
        value: cdktf.booleanToHclTerraform(this._addLocation),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      advertise_on_public_default_vip: {
        value: cdktf.booleanToHclTerraform(this._advertiseOnPublicDefaultVip),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      annotations: {
        value: cdktf.hashMapperHcl(cdktf.stringToHclTerraform)(this._annotations),
        isBlock: false,
        type: "map",
        storageClassType: "stringMap",
      },
      default_sensitive_data_policy: {
        value: cdktf.booleanToHclTerraform(this._defaultSensitiveDataPolicy),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      description: {
        value: cdktf.stringToHclTerraform(this._description),
        isBlock: false,
        type: "simple",
        storageClassType: "string",
      },
      disable: {
        value: cdktf.booleanToHclTerraform(this._disable),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      disable_api_definition: {
        value: cdktf.booleanToHclTerraform(this._disableApiDefinition),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      disable_api_discovery: {
        value: cdktf.booleanToHclTerraform(this._disableApiDiscovery),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      disable_api_testing: {
        value: cdktf.booleanToHclTerraform(this._disableApiTesting),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      disable_bot_defense: {
        value: cdktf.booleanToHclTerraform(this._disableBotDefense),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      disable_client_side_defense: {
        value: cdktf.booleanToHclTerraform(this._disableClientSideDefense),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      disable_ip_reputation: {
        value: cdktf.booleanToHclTerraform(this._disableIpReputation),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      disable_malicious_user_detection: {
        value: cdktf.booleanToHclTerraform(this._disableMaliciousUserDetection),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      disable_malware_protection: {
        value: cdktf.booleanToHclTerraform(this._disableMalwareProtection),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      disable_rate_limit: {
        value: cdktf.booleanToHclTerraform(this._disableRateLimit),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      disable_threat_mesh: {
        value: cdktf.booleanToHclTerraform(this._disableThreatMesh),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      disable_trust_client_ip_headers: {
        value: cdktf.booleanToHclTerraform(this._disableTrustClientIpHeaders),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      disable_waf: {
        value: cdktf.booleanToHclTerraform(this._disableWaf),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      do_not_advertise: {
        value: cdktf.booleanToHclTerraform(this._doNotAdvertise),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      domains: {
        value: cdktf.listMapperHcl(cdktf.stringToHclTerraform, false)(this._domains),
        isBlock: false,
        type: "list",
        storageClassType: "stringList",
      },
      enable_malicious_user_detection: {
        value: cdktf.booleanToHclTerraform(this._enableMaliciousUserDetection),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      enable_threat_mesh: {
        value: cdktf.booleanToHclTerraform(this._enableThreatMesh),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      id: {
        value: cdktf.stringToHclTerraform(this._id),
        isBlock: false,
        type: "simple",
        storageClassType: "string",
      },
      labels: {
        value: cdktf.hashMapperHcl(cdktf.stringToHclTerraform)(this._labels),
        isBlock: false,
        type: "map",
        storageClassType: "stringMap",
      },
      least_active: {
        value: cdktf.booleanToHclTerraform(this._leastActive),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      name: {
        value: cdktf.stringToHclTerraform(this._name),
        isBlock: false,
        type: "simple",
        storageClassType: "string",
      },
      namespace: {
        value: cdktf.stringToHclTerraform(this._namespace),
        isBlock: false,
        type: "simple",
        storageClassType: "string",
      },
      no_challenge: {
        value: cdktf.booleanToHclTerraform(this._noChallenge),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      no_service_policies: {
        value: cdktf.booleanToHclTerraform(this._noServicePolicies),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      random: {
        value: cdktf.booleanToHclTerraform(this._random),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      round_robin: {
        value: cdktf.booleanToHclTerraform(this._roundRobin),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      service_policies_from_namespace: {
        value: cdktf.booleanToHclTerraform(this._servicePoliciesFromNamespace),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      source_ip_stickiness: {
        value: cdktf.booleanToHclTerraform(this._sourceIpStickiness),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      system_default_timeouts: {
        value: cdktf.booleanToHclTerraform(this._systemDefaultTimeouts),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      user_id_client_ip: {
        value: cdktf.booleanToHclTerraform(this._userIdClientIp),
        isBlock: false,
        type: "simple",
        storageClassType: "boolean",
      },
      active_service_policies: {
        value: httpLoadbalancerActiveServicePoliciesToHclTerraform(this._activeServicePolicies.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerActiveServicePoliciesList",
      },
      advertise_custom: {
        value: httpLoadbalancerAdvertiseCustomToHclTerraform(this._advertiseCustom.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerAdvertiseCustomList",
      },
      advertise_on_public: {
        value: httpLoadbalancerAdvertiseOnPublicToHclTerraform(this._advertiseOnPublic.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerAdvertiseOnPublicList",
      },
      api_protection_rules: {
        value: httpLoadbalancerApiProtectionRulesToHclTerraform(this._apiProtectionRules.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerApiProtectionRulesList",
      },
      api_rate_limit: {
        value: httpLoadbalancerApiRateLimitToHclTerraform(this._apiRateLimit.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerApiRateLimitList",
      },
      api_specification: {
        value: httpLoadbalancerApiSpecificationToHclTerraform(this._apiSpecification.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerApiSpecificationList",
      },
      api_testing: {
        value: httpLoadbalancerApiTestingToHclTerraform(this._apiTesting.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerApiTestingList",
      },
      app_firewall: {
        value: httpLoadbalancerAppFirewallToHclTerraform(this._appFirewall.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerAppFirewallList",
      },
      blocked_clients: {
        value: cdktf.listMapperHcl(httpLoadbalancerBlockedClientsToHclTerraform, true)(this._blockedClients.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerBlockedClientsList",
      },
      bot_defense: {
        value: httpLoadbalancerBotDefenseToHclTerraform(this._botDefense.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerBotDefenseList",
      },
      captcha_challenge: {
        value: httpLoadbalancerCaptchaChallengeToHclTerraform(this._captchaChallenge.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerCaptchaChallengeList",
      },
      client_side_defense: {
        value: httpLoadbalancerClientSideDefenseToHclTerraform(this._clientSideDefense.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerClientSideDefenseList",
      },
      cookie_stickiness: {
        value: httpLoadbalancerCookieStickinessToHclTerraform(this._cookieStickiness.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerCookieStickinessList",
      },
      cors_policy: {
        value: httpLoadbalancerCorsPolicyToHclTerraform(this._corsPolicy.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerCorsPolicyList",
      },
      csrf_policy: {
        value: httpLoadbalancerCsrfPolicyToHclTerraform(this._csrfPolicy.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerCsrfPolicyList",
      },
      data_guard_rules: {
        value: cdktf.listMapperHcl(httpLoadbalancerDataGuardRulesToHclTerraform, true)(this._dataGuardRules.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerDataGuardRulesList",
      },
      ddos_mitigation_rules: {
        value: cdktf.listMapperHcl(httpLoadbalancerDdosMitigationRulesToHclTerraform, true)(this._ddosMitigationRules.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerDdosMitigationRulesList",
      },
      default_route_pools: {
        value: cdktf.listMapperHcl(httpLoadbalancerDefaultRoutePoolsToHclTerraform, true)(this._defaultRoutePools.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerDefaultRoutePoolsList",
      },
      enable_api_discovery: {
        value: httpLoadbalancerEnableApiDiscoveryToHclTerraform(this._enableApiDiscovery.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerEnableApiDiscoveryList",
      },
      enable_challenge: {
        value: httpLoadbalancerEnableChallengeToHclTerraform(this._enableChallenge.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerEnableChallengeList",
      },
      enable_ip_reputation: {
        value: httpLoadbalancerEnableIpReputationToHclTerraform(this._enableIpReputation.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerEnableIpReputationList",
      },
      enable_trust_client_ip_headers: {
        value: httpLoadbalancerEnableTrustClientIpHeadersToHclTerraform(this._enableTrustClientIpHeaders.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerEnableTrustClientIpHeadersList",
      },
      graphql_rules: {
        value: cdktf.listMapperHcl(httpLoadbalancerGraphqlRulesToHclTerraform, true)(this._graphqlRules.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerGraphqlRulesList",
      },
      http: {
        value: httpLoadbalancerHttpToHclTerraform(this._http.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerHttpList",
      },
      https: {
        value: httpLoadbalancerHttpsToHclTerraform(this._https.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerHttpsList",
      },
      https_auto_cert: {
        value: httpLoadbalancerHttpsAutoCertToHclTerraform(this._httpsAutoCert.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerHttpsAutoCertList",
      },
      js_challenge: {
        value: httpLoadbalancerJsChallengeToHclTerraform(this._jsChallenge.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerJsChallengeList",
      },
      jwt_validation: {
        value: httpLoadbalancerJwtValidationToHclTerraform(this._jwtValidation.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerJwtValidationList",
      },
      l7_ddos_protection: {
        value: httpLoadbalancerL7DdosProtectionToHclTerraform(this._l7DdosProtection.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerL7DdosProtectionList",
      },
      malware_protection_settings: {
        value: httpLoadbalancerMalwareProtectionSettingsToHclTerraform(this._malwareProtectionSettings.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerMalwareProtectionSettingsList",
      },
      more_option: {
        value: httpLoadbalancerMoreOptionToHclTerraform(this._moreOption.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerMoreOptionList",
      },
      origin_server_subset_rule_list: {
        value: httpLoadbalancerOriginServerSubsetRuleListStructToHclTerraform(this._originServerSubsetRuleList.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerOriginServerSubsetRuleListStructList",
      },
      policy_based_challenge: {
        value: httpLoadbalancerPolicyBasedChallengeToHclTerraform(this._policyBasedChallenge.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerPolicyBasedChallengeList",
      },
      protected_cookies: {
        value: cdktf.listMapperHcl(httpLoadbalancerProtectedCookiesToHclTerraform, true)(this._protectedCookies.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerProtectedCookiesList",
      },
      rate_limit: {
        value: httpLoadbalancerRateLimitToHclTerraform(this._rateLimit.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerRateLimitList",
      },
      ring_hash: {
        value: httpLoadbalancerRingHashToHclTerraform(this._ringHash.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerRingHashList",
      },
      routes: {
        value: cdktf.listMapperHcl(httpLoadbalancerRoutesToHclTerraform, true)(this._routes.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerRoutesList",
      },
      sensitive_data_disclosure_rules: {
        value: httpLoadbalancerSensitiveDataDisclosureRulesToHclTerraform(this._sensitiveDataDisclosureRules.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerSensitiveDataDisclosureRulesList",
      },
      sensitive_data_policy: {
        value: httpLoadbalancerSensitiveDataPolicyToHclTerraform(this._sensitiveDataPolicy.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerSensitiveDataPolicyList",
      },
      slow_ddos_mitigation: {
        value: httpLoadbalancerSlowDdosMitigationToHclTerraform(this._slowDdosMitigation.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerSlowDdosMitigationList",
      },
      trusted_clients: {
        value: cdktf.listMapperHcl(httpLoadbalancerTrustedClientsToHclTerraform, true)(this._trustedClients.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerTrustedClientsList",
      },
      user_identification: {
        value: httpLoadbalancerUserIdentificationToHclTerraform(this._userIdentification.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerUserIdentificationList",
      },
      waf_exclusion_rules: {
        value: cdktf.listMapperHcl(httpLoadbalancerWafExclusionRulesToHclTerraform, true)(this._wafExclusionRules.internalValue),
        isBlock: true,
        type: "list",
        storageClassType: "HttpLoadbalancerWafExclusionRulesList",
      },
    };

    // remove undefined attributes
    return Object.fromEntries(Object.entries(attrs).filter(([_, value]) => value !== undefined && value.value !== undefined ))
  }
}
